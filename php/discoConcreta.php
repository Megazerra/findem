...
while($fila = mysqli_fetch_assoc($r)){
	$amigo[] = Array('nombre' => $fila['nombre'],'idDiscoteca' => $fila['idDiscoteca'],'descripcion' => $fila['descripcion'],'logo' => "data:image/png;base64," . base64_encode($fila['logo']), 'direccion' => $fila['direccion'],'latitud' => $fila['latitud'],'longitud' => $fila['latitud'],'horario' => $fila['horario'], 'href' => $fila['href']);
	
}


foreach($amigo as $codigo => $ins){
	$todos[] = "<discoteca><idDiscoteca>".$ins['idDiscoteca']."</idDiscoteca><nombre>".$ins['nombre']."</nombre><descripcion>".$ins['descripcion']."</descripcion><logo>".$ins['logo']."</logo><direccion>".$ins['direccion']."</direccion><latitud>".$ins['latitud']."</latitud><latitud>".$ins['latitud']."</latitud><horario>".$ins['horario']."</horario><href>".$ins['href']."</href></discoteca>";
}
